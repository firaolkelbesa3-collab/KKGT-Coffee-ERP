    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative bg-white w-80 max-w-full h-full shadow-2xl flex flex-col overflow-y-auto z-10">
            ...
            <div className="space-y-1.5">
              {STATUSES.map(s => (
                <label key={s} className="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="checkbox" className="accent-[#126433]" checked={(draft.statuses || []).includes(s)} onChange={() => toggleStatus(s)} />
                  {s}
                </label>
              ))}
            </div>
        ...
        <div className="px-5 py-4 border-t flex gap-2">
          <Button className="flex-1 bg-[#f06721] hover:bg-[#d4591c] text-white" onClick={() => onApply(draft)}>Apply Filters</Button>
